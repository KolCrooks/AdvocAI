        ...
        for (let i = 0; i < state && i < messages.length; i++) {
            if (messages[i].startsWith('<input')){
                ...
                <div key={i} class="user-input">
                    <textarea autoFocus className="resize-none flex-wrap outline outline-1 text-right w-full outline-0 border-none font-mono"></textarea>
                    {
                        i === state-1 ?
            ...
            <Transition in={state === 0} timeout={duration}>
                {state => <div className="flex flex-col grow"
                    ...
                    <div className="flex flex-col justify-center items-center grow transition-all">
                        ...
                        <div className="pt-3">
                            <button className="font-serif bg-black text-white rounded-md p-2 m-4" onClick={()=>setState(1)}>Consult.</button>
                        </div>
                    </div>
